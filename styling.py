...
# By the time of scraping, the latest update on the webpage was on 18th Feb,
# further updates may be added to the webpage but not included in our
# scrape data.

# This webpage is a online 'bot' page
# created by Chinese female netizens where
# the bot receives and lists information about gender-specific crimes in China.
# Though it is not an official source, most of the
# documented crimes have urls to their news reports on
# mainstream media and are well-known on social media,
...
# removing crimes happened in Taiwan ('台湾') as it is controversial
# and our other datasets do not include Taiwan
...
#
# The json file is an outcome of the project
# here:https://github.com/adyliu/china_area
# the author used data from the Chinese statistic bearau
...
# The json file is an outcome of the project
# here:https://github.com/WenryXu/ChinaUniversity
# the author used data from the Education bureaus and WikiPedia
# which should be reliable.
...
# We first match crimes based on universities,
# because universities can be more precisely located,
# and there exist cases where universities are
# named after a city/province but located elsewhere.
...
for l in crime:
    ...
    if ('北大' in l or '清华' in l) and l in left_crime:
        ...
        # people do not call them by their full name but these abbreviations.
...
def county_match(s):
    ...
    if len(set(cities)) == 1:
        ...
        # abbreviation may not be find when searching through the sets,
...
# manually editing some wrongly matched crimes
...
ind = [i for i in range(len(lst_ma)) if lst_ma[i][0] ==
        '福建厦门一女患者在复旦大学附属医院厦门分院更衣室换衣服被男子观看全程，医院：不能说是偷窥！'][0]
lst_ma[ind] = ('福建厦门一女患者在复旦大学附属医院厦门分院更衣室换衣服被男子观看全程，医院：不能说是偷窥！','厦门市')
dct_m['厦门市'] += 1
dct_m['上海市'] -= 1
...
# construct a dictionary matching city Chinese names to English ones
dct_lang = {}
...
# We drop the left crimes because they cannot be further matched;
# we tried requesting the urls but as different webpages use different
# design, the retrived cities are quite inaccurate. Since these left
# crimes take up less than 9% of the total crimes, we believe dropping
# them would not largely affect our results.
